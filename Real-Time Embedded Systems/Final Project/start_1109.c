...
void *consumer (void *q)
{
    ...
    double milliseconds;
    struct timeval beforeMut;
    long long pre_mut;
    long long cons_del_time;
    
    while(1)
    {
        gettimeofday(&beforeMut, NULL);
        pre_mut = (long long)((long long)beforeMut.tv_sec * 1000000 + (long long)beforeMut.tv_usec);
        pthread_mutex_lock (fifo->mut);
        while (fifo->empty)
        {
